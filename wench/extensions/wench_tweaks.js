...
XKit.extensions.wench_tweaks = new Object({
			...
			if (data) {
				...
				GM_xmlhttpRequest({
					...
					onload: function(response) {
						let respdata = JSON.parse(response.responseText).response;
						if (respdata.total_posts != 1) {
							callback(false);
						} else {
	...
	getimageinfo: function(post, imgurl) {
		for (let ph = 0; ph < post.photos.length; ph++) {
			let photo = post.photos[ph];
			for (let al = 0; al < photo.alt_sizes.length; al++) {
				let alt = photo.alt_sizes[al];
				if (alt.url === imgurl) return { details: photo, size: al };
			}
		}
